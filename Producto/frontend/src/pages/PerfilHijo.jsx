...
function exportarAsistenciaPDF(alumno, asistencia) {
  const doc = new jsPDF();
  const hoy = new Date().toLocaleDateString('es-CL', { day: 'numeric', month: 'long', year: 'numeric' });
  doc.setFontSize(22); doc.setTextColor(99, 102, 241);
  doc.text('EduSync', 105, 20, { align: 'center' });
  ...
  if (asistencia.length > 0) {
    ...
    const pct = ((presentes / asistencia.length) * 100).toFixed(1);
    doc.setFontSize(13);
    doc.setTextColor(...(parseFloat(pct) >= 85 ? [16, 185, 129] : [239, 68, 68]));
    doc.text(`Asistencia total: ${pct}% (${presentes}/${asistencia.length} clases)`, 105, 80, { align: 'center' });
  }
  const estadoColor = { presente: [16, 185, 129], ausente: [239, 68, 68], tardanza: [245, 158, 11] };
  autoTable(doc, {
    startY: 88,
    head: [['Fecha', 'Estado', 'Justificación']],
    body: [...asistencia].sort((a, b) => new Date(b.fecha) - new Date(a.fecha)).map(r => [
    ...
    didParseCell: (d) => {
      if (d.column.index === 1 && d.section === 'body') {
        d.cell.styles.textColor = estadoColor[d.cell.text[0]] || [34, 34, 34];
        d.cell.styles.fontStyle = 'bold';
      }
    },
  });
  doc.save(`asistencia_${alumno.nombre_completo.replace(/ /g, '_')}.pdf`);
}
          ...
          <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.05 }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'flex-end' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                <button onClick={() => exportarNotasPDF(hijoSel, notas)}
                ...
                <button onClick={() => exportarAsistenciaPDF(hijoSel, asistencia)}
